...
export function parseLessonId(id, certifications = CERTIFICATIONS) {
    ...
    const cert = [...certifications]
        ...
        .find(item => id.startsWith(`${item.id}-`));
    if (!cert) return null;
    const sectionRef = id.slice(cert.id.length + 1);
    return sectionRef ? { certId: cert.id, sectionRef } : null;
}
...
async function initLessonPage() {
    ...
    document.title = `${lesson.title} - QA Learning Trainer`;
}
